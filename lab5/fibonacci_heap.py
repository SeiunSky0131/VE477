import math
class FiboNode:
    def __init__(self, key_t, struct_t):
        self.struct = struct_t   #define the name of the node/ note that this can be used for any type in convinence 
        self.parent = None
    ...
    #return the node with struct = struct_t in the heap with the left most node in the root linked list to be self
    #return None if the node is not found
    def Find_Node_by_Struct(self, struct_t):      
        curr_node = self            #set the current node to be the left most node in the root linked list
        end_node = self.left        #set the end node to be the left of current node

        while True:
            # print("curr_node in the loop is:",curr_node.struct)
            #if the current node has the struct, then we are done
            if(curr_node.struct == struct_t): 
                return curr_node
            
            #if the we search all the children of current node and we find the struct, then we are done
            if(curr_node.child != None): 
                result = curr_node.child.Find_Node_by_Struct(struct_t)
                if (result != None):
                    return result
...
class FiboHeap:
    ...
    #Return a pointer to a node with struct to be struct_t
    def FindNode_in_FiboHeap_by_Struct(self,struct_t):
        return self.min.Find_Node_by_Struct(struct_t)